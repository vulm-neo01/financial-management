...
function GroupCreateSpendModal({ onCreateExchange, open, onClose, groupWalletId }) {
    ...
    const handleFormChange = (event) => {
        ...
        if (name === 'amount' && value > groupAmount) {
            setWarningOverAmount(`The amount exceeds the current wallet balance: ${value.toLocaleString()} > ${groupAmount.toLocaleString()}`);
        } else {
    ...
    const handleCreateExchange = async () => {
        ...
        try {
            ...
            const formUploadResponse = await request("post", "/group/exchanges", () => {
                // console.log("success")
            }, (err) => {
                console.log("failure: " + err)
            }, updatedFormData);
    ...
    useEffect(() => {
        request("get", `/group/budgets/all/${groupWalletId}`, (res) => {
            const category = res.data.filter(c => c.type === "spend");
            ...
            >
                ...
                >
                    ...
                    <FormControl fullWidth sx={{ mb: 1 }}>
                        ...
                        <Input tabIndex={2} id="to" name="to" value={formData.to} onChange={handleFormChange} />
                    </FormControl>
                    <FormControl fullWidth sx={{ mb: 1 }}>
...
export default GroupCreateSpendModal;